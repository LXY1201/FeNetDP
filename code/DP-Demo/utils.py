...
def generate_network_architecture(graph_dicts, drug_gene_dicts, key1, key2, thres):
    ...
    focus_genes_list = []
    with open('./data/gene_ferroptosis_8.txt') as f:
...
def read_global_graph(thres_score):
    ...
    with open('./data/9606.protein.info.v12.0.txt') as f:
    ...
    graph_dicts = {}
    graph_dicts['nodes'] = {}
    with open('./data/9606.protein.links.full.v12.0.txt') as f:
        for idx, line in enumerate(f.readlines()):
            if idx == 0:
                continue
            line = line.split()
...
def read_patient_info(gene_list):
    csv_reader = csv.reader(open("./data/CPTAC_gene_matrix.csv"))
    first_line = True
    X_index = {}
    ...
    csv_reader = csv.reader(open("./data/CPTAC_info.csv"))
    first_line = True
    y_tumor = {} # 1-tumor 0-normal
    y_subtypes = {} # 1-Tumor  0-Normal
    for line in csv_reader:
        if first_line:
            first_line = False
        else:
    ...
    first_line = True
    pos_needed = ["PATIENT_ID", "OS_STATUS", "OS_TIME_DAYS", "OS_TIME_MONTHS"]
    pos_id_needed = []
    y_survival = {}
    with open("./data/CPTAC_clinical_patient.txt","r") as f:
        for line in f:
